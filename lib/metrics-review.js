...
function formatConclusionMarkdown(ctx) {
  ...
  lines.push(`# Baking — conclusión ciclo ${state.cycleId}`);
  lines.push('');
  lines.push(`- **Generado:** ${writtenAt}`);
  lines.push(`- **Disparo:** ${trigger === 'runs' ? `${cfg.minRuns} corridas` : trigger === 'time' ? `${cfg.intervalDays} días` : 'corridas + tiempo'}`);
  lines.push(`- **Runs en ciclo:** ${due.runsInCycle} (total archivo: ${routing.totalRuns})`);
  ...
  lines.push(`|---------|-------|`);
  lines.push(`| Veredicto | **${routing.verdict}** |`);
  lines.push(`| Muestra auditada | ${routing.sampleSize} últimas corridas |`);
  lines.push(`| underkill/overkill en muestra | ${routing.badCount} (${routing.badPct ?? '—'}%) |`);
  lines.push(`| Umbral OK | ≤ ${routing.maxBadPct}% |`);
  lines.push('');
  lines.push('Distribución `plan_fit` (todo el ciclo):');
  lines.push('```json');
  lines.push(JSON.stringify(routing.counts.plan, null, 2));
  lines.push('```');
  lines.push('');
  lines.push('Distribución `exec_fit` (todo el ciclo):');
  lines.push('```json');
  ...
  lines.push('');
  lines.push('## Ahorro (bench emparejado)');
  lines.push('');
  lines.push(`| Métrica | Valor |`);
  lines.push(`|---------|-------|`);
  lines.push(`| Veredicto | **${savings.verdict}** |`);
  lines.push(`| Pares con costo | ${savings.pairsWithUsage} |`);
  lines.push(`| Pares ≥ ${savings.minPct}% ahorro | ${savings.pairsMeetingTarget} (mín ${savings.minPairs}) |`);
  lines.push(`| Ahorro promedio | ${savings.avgSavingsPct != null ? savings.avgSavingsPct + '%' : '—'} |`);
  ...
  if (routingOk && savingsOk) {
    lines.push(
      '**Objetivo cumplido** en routing y ahorro bench. Podés cerrar el ciclo:'
    );
    lines.push('');
    lines.push('```bash');
    ...
    lines.push('```');
    lines.push('');
    lines.push('Eso archiva `runs.jsonl` y arranca ciclo nuevo. No borres el archivo archivado.');
  } else if (routingOk && savings.verdict === 'NO_BENCH_DATA') {
    lines.push(
      '**Routing OK** en la muestra. **Ahorro no medido** — corré bench S0 vs S3 con `pair_id` + `usage.total_usd` si querés validar costo.'
    );
  } else if (routingOk) {
    lines.push('**Routing OK.** Seguí acumulando bench de ahorro o cerrá ciclo si solo te importa routing.');
  } else {
    lines.push(
      '**Ajustar reglas** — revisá casos underkill/overkill y señales en `BAKING-CURSOR.md` / config antes de cerrar ciclo.'
    );
  }

  lines.push('');
  lines.push('---');
  lines.push('*Regenerar: `baking metrics-review --force` · Estado: `baking metrics-review --status`*');
...
function formatStatus(ctx) {
  ...
  lines.push(`Runs en ciclo: ${due.runsInCycle} / ${cfg.minRuns} (trigger)`);
  lines.push(`Días desde última revisión/inicio: ${due.daysSinceAnchor} / ${cfg.intervalDays}`);
  lines.push('');
  if (due.due) {
    lines.push('Estado: LISTO para conclusión → correr `baking metrics-review`');
    lines.push(`Disparo: ${due.trigger}`);
  } else {
    lines.push('Estado: acumulando');
    lines.push(`Faltan ~${due.runsRemaining} runs o ~${due.daysRemaining} días`);
  }
  return lines.join('\n');
}
...
function closeCycle(paths, state) {
  if (!fs.existsSync(paths.runsFile)) {
    throw new Error(`No existe ${paths.runsFile}`);
  }
...
function runReview(options = {}) {
  ...
  if (!fs.existsSync(paths.runsFile)) {
    return {
      ok: false,
      error: `No hay métricas en ${paths.runsFile}. Corré /baking al menos una vez.`,
    };
  }
  ...
  if (closeCycleFlag) {
    ...
    return {
      ok: true,
      closed: true,
      message: `Ciclo ${state.cycleId} archivado → ${result.archiveName}. Nuevo ciclo ${result.newState.cycleId}.`,
...
function formatReportShort(routing, savings, due) {
  return [
    `Routing: ${routing.verdict} (${routing.badPct ?? '—'}% bad en muestra de ${routing.sampleSize})`,
    `Ahorro: ${savings.verdict}`,
    `Trigger: ${due.trigger}`,
  ].join(' · ');
}
