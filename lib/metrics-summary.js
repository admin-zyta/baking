...
function comparePairs(rows) {
  ...
  for (const [pairId, pairRows] of pairs) {
    ...
    if (baseUsd == null || bakeUsd == null) {
      out.push({ pair_id: pairId, note: 'Missing usage.total_usd on one or both arms' });
      continue;
    }
...
function formatReport(report) {
  ...
  lines.push(`Runs: ${report.total_runs} (${report.runs_with_usage} with usage.total_usd)`);
  lines.push('');

  if (report.scenarios.length) {
    lines.push('By scenario (benchmark.scenario_id):');
  ...
  if (report.arms.length) {
    lines.push('By arm (benchmark.arm):');
  ...
  if (report.pair_comparisons.length) {
    lines.push('Paired comparisons (benchmark.pair_id):');
