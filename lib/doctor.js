...
function doctor() {
  ...
  const configRoot = path.join(home, '.cursor', 'opus-sonnet');
  ...
  const configOk = fs.existsSync(path.join(configRoot, 'config.json'));
  ...
  return {
    ...
    hint: ok
      ? null
      : 'Corré: node bin/baking.js install (desde el clone del repo o npx @boogiepop/baking install)',
  };
}
